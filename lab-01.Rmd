...
visitors %>% 
  summarise(
    ...
    med_2021 = mean(n_2021, na.rm = TRUE)
  )
```
...
NA means "Not Available", there may have been some attractions that were not avaiable in 2021. 
...
na.rm is a condition that allows NA values to not be taken into account, e.g. 'na.rm = TRUE' will allow the mean to be calculated without NA values being taken into account.
...
**Write your answer here**
The data is positively skewed, where the mean value is higher than the median value. This suggests the data is not symmetrical where the mean value is the same as the median.
